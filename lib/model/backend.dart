...
class Backend {
  ...
  /// Returns all odontologo.

    List<Odontologo> getOdontologos() {
    return _odontologos;
  }

  /// Marks odontologo identified by its id as read.
  void markOdontologoAsRead(int id) {
    final index = _odontologos.indexWhere((odontologo) => odontologo.id == id);
    _odontologos[index].read = true;
  }

  /// Deletes odontologo identified by its id.
  void deleteOdontologo(int id) {
    _odontologos.removeWhere((odontologo) => odontologo.id == id);
  }
